...
- Name: TC2_LoginUser.test.ts >> Login Test >> Valid Login
- Location: tests\TC2_LoginUser.test.ts:9:9
...
Locator: getByText('Returning Customer')
Expected: visible
Error: strict mode violation: getByText('Returning Customer') resolved to 2 elements:
    1) <h2>Returning Customer</h2> aka getByRole('heading', { name: 'Returning Customer' })
    2) <strong>I am a returning customer</strong> aka getByText('I am a returning customer')

Call log:
  - Expect "toBeVisible" with timeout 5000ms
  - waiting for getByText('Returning Customer')
...
```ts
  ...
  5  |     readonly loginPageTitle: Locator;
  6  |     readonly email: Locator;
  7  |     readonly password: Locator;
  8  |     readonly loginButton: Locator;
  9  |     readonly loggedIn: Locator;
  10 | 
  11 |     constructor(page: Page){
  12 |         this.page = page;
  13 |         this.loginPageTitle = page.getByText("Returning Customer");
  14 |         this.email = page.locator("input#input-email");
  15 |         this.password = page.locator("input#input-password");
  16 |         this.loginButton = page.locator("//input[@type='submit']");
  17 |         this.loggedIn = page.getByRole('heading', {name: "My Account"});
  18 |     }
  19 | 
  20 |     async verifyLoginTitle(){
> 21 |         await expect(this.loginPageTitle).toBeVisible();
     |                                           ^ Error: expect(locator).toBeVisible() failed
  22 |     }
  23 | 
  24 |     async enterCredentials(E_Mail: string, Password: string){
  25 |         await this.email.fill(E_Mail);
  26 |         await this.password.fill(Password);
  27 |     }
  28 | 
  29 |     async clickLoginButton(){
  30 |         await this.loginButton.click();
  31 |     }
  32 | 
  33 |     async verifyLoginStatus(){
  34 |         await expect(this.loggedIn).toBeVisible();
  35 |     }
  36 | }
```